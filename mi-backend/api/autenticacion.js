...
module.exports = function (db) {
  ...
  //************************************************************************* */
  //*********************ACCESO A PROPIETARIOS AL SISTEMA******************** */
  //************************************************************************* */

    router.post('/login-propietario', (req, res) => {
        ...
        const query = `
            SELECT u.*, p.* 
            FROM usuarios u 
            INNER JOIN propietarios p ON u.doc = p.id_prop 
            WHERE u.email = ?
        `;
        db.query(query, [email], async (err, results) => {
            if (err) {
                console.error('Error en login propietario:', err);
            ...
            if (results.length === 0) {
                return res.status(401).json({ 
                    success: false,
                    message: 'Correo o contraseña incorrectos o no es propietario' 
                });
            }
            ...
            return res.status(200).json({
                ...
                redirect: '/UserWelcome' // Ruta de redirección específica para propietarios
            });
        });
    });
